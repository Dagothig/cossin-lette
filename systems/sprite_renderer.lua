...
function check_imgs(world, sheet)
    for _, entry in ipairs(sheet) do
        local img = world.components.img.get(entry.src)
        if not img then
            img = love.graphics.newImage('img/' .. entry.src)
            world.components.img.set(entry.src, img)
        end
        if entry.strips then
            goto continue
        end
...
function sprite_renderer.load(world)
    for _, sheet in world.components.sheets.iter() do
        check_imgs(sheet)
    end
end
...
function sprite_renderer.draw(world)
    ...
    for sprite in world.components.sprites.iter({ min = { 0, 0 }, max = { width, height } }) do
        local sheet = world.components.sheets.get[sprite.entity]
